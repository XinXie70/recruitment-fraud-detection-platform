...
def run_training(
    ...
    logger.info(
        ...
        "(fraud_f1=%.4f recall=%.4f mode=%s min_recall=%s step=%s)",
        best_thr,
        val_metrics_opt["fraud_f1"],
        val_metrics_opt["fraud_recall"],
        "min_recall_then_f1" if cfg.min_fraud_recall_for_threshold is not None else "max_fraud_f1",
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--class_weight_max", type=float, default=5.0)
    p.add_argument("--fraud_oversample_factor", type=float, default=3.0)
    ...
    p.add_argument("--min_fraud_recall_for_threshold", type=float, default=0.85)
