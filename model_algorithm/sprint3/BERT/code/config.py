...
import json, logging, os, random, re, sys, time, torch
...
TEXT_FIELDS: List[str] = ['title', 'company_profile', 'description', 'requirements', 'benefits']
OPTIONAL_CONTEXT_FIELDS: List[str] = ['employment_type', 'required_experience', 'required_education', 'industry', 'function', 'location', 'department', 'salary_range']
TAGGED_FIELD_ORDER: List[str] = ['title', 'company_profile', 'description', 'requirements', 'benefits', 'employment_type', 'required_experience', 'industry', 'function']
FIELD_TAG_MAP: Dict[str, str] = {'title': 'TITLE', 'company_profile': 'COMPANY PROFILE', 'description': 'DESCRIPTION', 'requirements': 'REQUIREMENTS', 'benefits': 'BENEFITS', 'employment_type': 'EMPLOYMENT TYPE', 'required_experience': 'REQUIRED EXPERIENCE', 'industry': 'INDUSTRY', 'function': 'FUNCTION'}
...
def artifact_slug(text: str) -> str:
    slug = re.sub('[^a-z0-9]+', '_', str(text).strip().lower())
...
def ensure_directories() -> None:
    for path in (WEIGHTS_DIR, BERT_FINETUNED_DIR, RESULTS_DIR, FIGURES_DIR): path.mkdir(parents=True, exist_ok=True)
...
def default_paths_dict() -> Dict[str, str]:
    return {'bert_code_root': str(BERT_CODE_ROOT), 'project_root': str(PROJECT_ROOT), 'train_csv': str(TRAIN_CSV), 'validation_csv': str(VALIDATION_CSV), 'test_csv': str(TEST_CSV), 'weights_dir': str(WEIGHTS_DIR), 'results_dir': str(RESULTS_DIR), 'figures_dir': str(FIGURES_DIR), 'logs_dir': 'console_only', 'pretrained_model_name': PRETRAINED_MODEL_NAME, 'text_fields': TEXT_FIELDS, 'tagged_field_order': TAGGED_FIELD_ORDER, 'use_tagged_format': USE_TAGGED_FORMAT, 'prefer_combined_text': PREFER_COMBINED_TEXT}
ENSEMBLE_VAL_COPY = PROJECT_ROOT.parent / 'ensemble_BERT_FP' / 'results' / 'bert_validation_predictions.csv'
...
def collect_environment_info() -> Dict[str, Any]:
    info: Dict[str, Any] = {'python_executable': sys.executable, 'python_version': sys.version.replace('\n', ' '), 'pytorch_version': None, 'transformers_version': None, 'imbalanced_learn_version': None, 'sklearn_version': None, 'numpy_version': np.__version__, 'cuda_available': False, 'pytorch_cuda_version': None, 'cudnn_version': None, 'gpu_name': None, 'gpu_count': 0, 'current_training_device': 'cpu'}
    try:
        import torch
...
def print_environment_banner(logger: Optional[logging.Logger]=None) -> Dict[str, Any]:
    ...
    lines = ['======== Environment ========', f"Python executable      : {info['python_executable']}", f"Python version         : {info['python_version']}", f"PyTorch version        : {info['pytorch_version']}", f"Transformers version   : {info['transformers_version']}", f"CUDA available         : {info['cuda_available']}", f"PyTorch CUDA version   : {info['pytorch_cuda_version']}", f"cuDNN version          : {info['cudnn_version']}", f"GPU name               : {info['gpu_name']}", f"GPU count              : {info['gpu_count']}", f"Current training device: {info['current_training_device']}", '=============================']
    for line in lines:
...
def require_cuda_for_training(logger: Optional[logging.Logger]=None) -> 'torch.device':
    ...
    if not torch.cuda.is_available():
        msg = f"CUDA is not available. Formal training must use the E: CUDA environment.\nCurrent sys.executable = {sys.executable}\nPyTorch version = {info.get('pytorch_version')}\nCUDA available = False\nPlease activate the E: CUDA env, for example:\n  . E:\\ml\\activate.ps1\n  # or run with:\n  E:\\path\\to\\cuda_environment\\python.exe BERT/code/run.py train\nTraining aborted (will not fall back to CPU for long runs)."
        if logger is not None:
            logger.error(msg)
        else:
