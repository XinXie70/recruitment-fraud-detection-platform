...
MIN_DISPLAY_ABS_CONTRIBUTION = 1e-6
...
def _top_evidence(
    ...
    *,
    ...
    if merge_shap_tokens:
        ...
        candidates = _contextualize_noisy_items(candidates, text)
    else:
        candidates = [
            item
...
class XAIService:
    """Explain the exact FP-gate risk scorer with word or hierarchical SHAP."""
    ...
    def _explain_hierarchical_shap(
        ...
        total_budget = min(self.max_evals, self.hierarchical_max_evals)
        coarse_budget = max(8, int(total_budget * 0.4))
        coarse_attribution = self._partition_attributions(
            text,
            score_batch,
            lambda value: _build_coarse_segments(
                value,
                self.hierarchical_segment_words,
            ),
            coarse_budget,
        ...
        ranked_segments = sorted(
            (
                ...
                for segment, contribution in zip(
                    coarse_attribution.segments,
                    coarse_attribution.contributions,
                    strict=True,
                )
                if abs(contribution) >= 1e-6
            ),
        ...
        remaining_budget = max(1, total_budget - coarse_budget)
        affordable_segments = max(1, remaining_budget // 8)
        selected_segments = ranked_segments[
            : min(self.hierarchical_top_segments, affordable_segments)
        ]
        if not selected_segments:
            return XAIResult(
                status="success",
                method="shap_partition",
                base_value=coarse_attribution.base_value,
                output_value=expected_output,
                items=[],
                message=(
                    "Hierarchical SHAP found no non-zero sentence or paragraph "
                    "contributions."
                ),
            )

        fine_budget = max(8, remaining_budget // len(selected_segments))
        fine_items: list[EvidenceSpan] = []
        for selected, _coarse_contribution in selected_segments:
            selected_text = text[selected.start : selected.end]

            def contextual_score_batch(
                values: Sequence[str],
                segment: TextSegment = selected,
            ) -> list[float]:
                contextual_values = [
                    f"{text[:segment.start]}{value}{text[segment.end:]}"
                    for value in values
                ]
                return score_batch(contextual_values)

            fine_attribution = self._partition_attributions(
                selected_text,
                contextual_score_batch,
                lambda value: _build_segments(value, max_segments=100000),
                fine_budget,
            )
            for local_segment, contribution in zip(
                fine_attribution.segments,
                fine_attribution.contributions,
                strict=True,
            ):
                if abs(contribution) < 1e-6:
                    continue
                start = selected.start + local_segment.start
                end = selected.start + local_segment.end
                fine_items.append(
                    EvidenceSpan(
                        text=text[start:end],
                        start=start,
        ...
        return XAIResult(
            ...
            message=(
                "Hierarchical SHAP selected "
                f"{len(selected_segments)} of {len(coarse_attribution.segments)} "
                "sentence or paragraph segments for word-level attribution."
            ),
        )
