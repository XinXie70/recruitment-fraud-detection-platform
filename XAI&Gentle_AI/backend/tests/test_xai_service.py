"""Tests for SHAP-only word and hierarchical XAI."""
...
from xai_gentle.xai_service import _build_coarse_segments, _top_evidence
...
def test_hierarchical_shap_returns_original_word_offsets() -> None:
    ...
    assert result.method == "shap_partition"
    assert result.version == "xai-v2"
    assert (result.message or "").startswith("Hierarchical SHAP selected")
    assert any(
        term in {item.text.lower() for item in result.items}
