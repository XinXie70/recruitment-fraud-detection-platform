...
def train_final_model(
    ...
    weights_path = Path(weights_dir) if weights_dir is not None else Path(
        getattr(cfg, "WEIGHTS_DIR", output_dir)
    )
    weights_path.mkdir(parents=True, exist_ok=True)

    model_path = weights_path / "final_model.keras"
    model.save(model_path)
    joblib.dump(vectorizer, weights_path / "tfidf_vectorizer.pkl")
    if reducer is not None:
        joblib.dump(reducer, weights_path / "dimensionality_reducer.pkl")
    ...
    with open(weights_path / "training_config.json", "w", encoding="utf-8") as f:
        json.dump(train_config, f, ensure_ascii=False, indent=2)
    # 结果目录也留一份配置副本，方便只看 results 时查阅
