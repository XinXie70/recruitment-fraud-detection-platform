...
def apply_config_to_shared_modules() -> None:
    """
    将本版本 config 覆盖写入 dnn_fraud.config，
    使复用的 train/features/model/metrics 模块读取相同超参数与 OUTPUT_DIR。
    """
    import models.dnn_fraud.config as shared
    ...
    # 同步路径常量（from * import 后 OUTPUT_DIR 等已在 cfg 中）
