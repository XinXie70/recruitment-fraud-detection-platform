"""Bi-LSTM training (Tokenized Sequences + Bidirectional LSTM + Class Weighting)."""
...
def build_bilstm_model(vocab_size: int, max_len: int) -> keras.Model:
    model = models.Sequential(name="FraudBiLSTM")
    model.add(layers.Input(shape=(max_len,)))
    model.add(layers.Embedding(vocab_size, EMBEDDING_DIM, name="embedding"))
    model.add(layers.Bidirectional(layers.LSTM(LSTM_UNITS, name="lstm"), name="bilstm"))
